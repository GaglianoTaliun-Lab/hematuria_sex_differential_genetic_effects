...
home_dir="/home/fridald4/links/projects/def-gsarah"
...
source("/scratch/fridald4/col4a2_hematuria/convert_loc_to_rs.R")
...
file_path <- file.path(project_dir, "datasets", 
    "GCST90301700.h.DBP_males.tsv.gz")
fread(file_path, nrows = 6)

DBP_m <- fread(file_path) %>%
    separate(data = ., col = pos.hg19, into = c("CHR", "BP"), sep = ":") %>%
    filter(., CHR == 13 & BP >= min_bp & BP <= max_bp) %>%
    convert_loc_to_rs(., dbsnp_144) %>%
    dplyr::mutate(varbeta = standard_error^2,
                  MAF = case_when(
                    effect_allele_frequency >= 0.5 ~ 1-effect_allele_frequency,
                    effect_allele_frequency < 0.5 ~ effect_allele_frequency
...
fwrite(DBP_m, file = here(project_dir, "data_for_coloc",stringr::str_c("DBP_males", out_ext)), sep = "\t")
...
file_path <- file.path(project_dir, "datasets", 
    "GCST90301703.h.PP_males.tsv.gz")
fread(file_path, nrows = 6)

PP_m <- fread(file_path) %>%
    separate(., pos.hg19, c("CHR", "BP"), sep = ":") %>%
    filter(., CHR == 13 & BP >= min_bp & BP <= max_bp) %>%
    convert_loc_to_rs(., dbsnp_144) %>%
    dplyr::mutate(varbeta = standard_error^2,
...
fwrite(PP_m, file = here(project_dir, "data_for_coloc",stringr::str_c("PP_males", out_ext)), sep = "\t")
