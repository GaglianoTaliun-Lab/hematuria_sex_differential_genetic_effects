...
gmirror_v2 <- function(top, bottom, tline, bline, chroms = c(1:23, "X", "Y"),log10=TRUE, 
  ...
  #Sort data
  colnames(top) <- c("SNP", "CHR", "POS", "pvalue")
  colnames(bottom) <- c("SNP", "CHR", "POS", "pvalue")
  topn <- names(top)
  bottomn <- names(bottom)
  top$Location <- "Top"
  bottom$Location <- "Bottom"

  top <- top %>% filter(., !is.na(pvalue))
  bottom <- bottom %>% filter(., !is.na(pvalue))
  
  ylim_set <- -log10(min(min(top$pvalue), min(bottom$pvalue)))

  top <- filter(top, pvalue < 0.1)
  bottom <- filter(bottom, pvalue < 0.1)
  ...
  yaxismax1 <- ifelse(freey==FALSE, ylim_set+1, max(d_order$pval[which(d_order$pval< Inf) & d_order$Location=="Top"]))
  yaxismax2 <- ifelse(freey==FALSE, ylim_set+1, max(d_order$pval[which(d_order$pval< Inf) & d_order$Location=="Bottom"]))
  ...
  if(!missing(tline)){
    for(i in 1:length(tline)){
      p1 <- p1 + geom_hline(yintercept = tredline[i], colour="black")
    }
  }
  if(!missing(bline)){
    for(i in 1:length(bline)){
      p2 <- p2 + geom_hline(yintercept = bredline[i], colour="black")
    }
  }
  ...
  # ggsave(p, filename=(file, ".", type), dpi=res, units="in", height=hgt, width=wi)
  return(p)
}
